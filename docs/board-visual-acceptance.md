...
The home screen now exposes `Review full board shell`, a read-only browser surface that renders all 254 candidate faces in the same landscape coordinate system. It uses separated cream faces, supports keyboard/click selection of an individual face, and exposes that face's canonical key, coordinate, verification state, feature state, and source references in a review inspector. It has no placeholder labels, dots, feature art, or playable commands; the warning identifies it as review tooling rather than MVP play. This improves cell-by-cell transcription without promoting unresolved physical content.
