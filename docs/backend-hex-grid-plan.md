...
- `apps/web/src/main.tsx` renders the complete neutral staggered 14-row visual lattice (24 faces per row, 336 total) for board orientation and overlays the nine `DEVELOPMENT_BOARD.hexes` as interactive development spaces; the lattice cells are explicitly non-authoritative and carry no inferred physical-board water, feature, or movement data.
