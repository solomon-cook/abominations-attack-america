...
On 2026-08-26, `npm run browser:online:verify` drove two fresh Chrome profiles against the local Vite client and memory API. Browser one created room `82946A`; browser two joined it; the harness completed all eight ordered setup choices, clicked **Ready** in both sessions, verified both reached the synchronized Move phase, then reloaded browser two and verified it recovered the same Move phase. This proves local online setup, readiness, shared phase, and reload recovery. It does not prove online movement, disconnect/reconnect, victory, persistence, production board promotion, or deployed-browser acceptance.
