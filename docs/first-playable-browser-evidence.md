...
MVP and provisional production room creation now use `createMvpRoomGame` with the pinned 245-cell `PROVISIONAL_AUTHORITATIVE_BOARD`; both Memory and Prisma room stores use that constructor unless a test explicitly opts into `allowDevelopmentFixture`. The browser labels the board as a best-guess provisional release while physical source transcription remains incomplete. Strict verified-board validation still rejects provisional content.
...
| Shared match board | Engine, API, and browser record the same board ID and content hash; provisional production room creation succeeds only for that pinned board | Verified for provisional promotion; source-faithful board remains blocked |
...
| Release evidence | `npm run verify`, deployment health, durable persistence/restart, monitoring, and production smoke evidence | Blocked: deployment and human release evidence remain open |
...
On 2026-08-26, `npm run browser:ci:verify` ran from a clean CI-started Vite server. The read-only board-review route reported 245 separated cream faces in 14 staggered rows at 1280x720, 834x1112, and 390x844, with zero horizontal overflow, zero playable tiles, zero unresolved labels, and an individual-cell inspector at each viewport. The local interaction matrix also passed setup, disabled unreachable destinations, path cancellation and confirmation, pending-action loading, Fight, Encounter, Deploy, and concession-terminal checks at all three sizes. This is automated Chrome evidence for the development fixture and review shell; it does not promote the physical board, establish cross-engine compatibility, or replace human visual, touch, zoom, keyboard, or screen-reader acceptance.
...
The same harness was rerun from a clean ephemeral-server start on 2026-08-26 and returned `ok=true`: setup, spectator no-act, process-disconnect reconnect, forged and malformed command rejection without state change, synchronized Move, Fight, Encounter, Deploy, concession, player and spectator terminal projection, and terminal reload recovery passed. The returned `onlineFight` value was `verified` after the harness selected the legal destination containing the mixed-choice setup deployment. This is still local memory/API development-fixture evidence, not remote or production evidence.

The mixed-choice rerun also covered one player choosing **Development Deploy** and one choosing **Draw Research**. Setup now materializes the selected lairs, starting Research hand, and valid branch-base deployment before the shared Move phase; the board and rule data remain explicitly provisional.
...
On 2026-08-26, `npm run browser:online:verify` completed a fresh three-session Chrome run against the normal online MVP room path. It verified public room creation, second-player join, spectator no-act projection, ordered mixed setup with initial deployment and Research, Ready, the pinned full-board room state, identical board ID/version/content/rendered hashes across both players and the spectator, exactly 245 rendered `.hex-tile` elements with no visible `Unresolved` labels in each session, WebSocket-process restart and reconnect, forged and malformed command rejection without state change, synchronized movement into the deployed unit’s space, Fight, Encounter, Deploy, concession, player/spectator terminal projection, and terminal reload recovery. Its result reported `onlineFight: verified`. This is desktop Chrome evidence for the best-guess MVP board; mobile support, manual acceptance, physically verified board data, and final release sign-off remain open.
...
The browser uses a generated decorative sea-and-land treatment beneath the interactive 245-cell overlay. It contains no board keys or rule data; the engine still supplies all current legal actions and recorded features. Internal reference photographs are not shipped in the web client.

## Current geometry correction

After reviewing the supplied comparison image and the responsive-grid reference, the candidate layout contract was tightened. The shared layout now uses flat-top faces, a three-quarter-width horizontal centre pitch, and a half-face-height offset on alternate columns. `npm run web-board-layout:verify` checks polygon-level interior intersections across all 245 candidate cells. This verifies the corrected visible honeycomb presentation, not physical-board topology or MVP promotion.
