...
On 2026-08-26, `npm run browser:online:verify` drove two player profiles and one spectator profile against the local Vite client and memory API. Browser one created a room; browser two joined it; the spectator joined with the room code; the harness verified the spectator received setup and Move projections with all rule-bearing controls disabled, completed all eight ordered setup choices, clicked **Ready** in both player sessions, verified synchronized Move, terminated and reopened browser one to force a WebSocket/process disconnect, restored its persisted room session, and recovered the same Move state before acting. It then submitted one forged-actor command and confirmed the API rejected it without changing the visible Move phase, reloaded browser two and recovered the same Move phase, selected and confirmed one legal movement in browser one, and synchronized all three sessions through Encounter and Deploy back to Move. This proves local online setup, spectator no-act projection, WebSocket/process disconnect and reconnect recovery, one forged-command rejection, readiness, shared phase, reload recovery, synchronized movement, Encounter, and Deploy progression. It does not prove Fight progression, victory, persistence, production board promotion, or deployed-browser acceptance.
