...
The same harness was rerun from a clean ephemeral-server start on 2026-08-26 and returned `ok=true`: setup, spectator no-act, process-disconnect reconnect, forged and malformed command rejection without state change, synchronized Move, Encounter, Deploy, concession, player and spectator terminal projection, and terminal reload recovery passed. The returned `onlineFight` value was `not-reached` because the normal MVP setup has no board units after its starting choices, so the selected movement did not create a battle. Online Fight progression remains an explicit coverage gap; this is still local memory/API development-fixture evidence, not remote or production evidence.
