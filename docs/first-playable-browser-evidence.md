...
On 2026-08-26, `npm run browser:online:verify` drove two player profiles and one spectator profile against the local Vite client and memory API. Browser one created a room; browser two joined it; the spectator joined with the room code; the harness verified the spectator received setup and Move projections with all rule-bearing controls disabled, completed all eight ordered setup choices, clicked **Ready** in both player sessions, verified synchronized Move, reloaded browser two and recovered the same Move phase, then selected and confirmed one legal movement in browser one. All three sessions observed the same settled `Encounter` phase after the transient server acknowledgement cleared; the harness followed the available Encounter decision chain, reached Deploy, passed deployment, and verified the player sessions returned to the next Move phase. This proves local online setup, spectator no-act projection, readiness, shared phase, reload recovery, synchronized movement, Encounter, and Deploy progression. It does not prove disconnect/reconnect, Fight progression, victory, persistence, production board promotion, or deployed-browser acceptance.
