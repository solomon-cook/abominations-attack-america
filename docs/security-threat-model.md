...
| Runtime dependencies | A vulnerable transitive package is exploitable through a tool or service boundary | `npm audit --omit=dev` was run on 2026-08-26; it reports three high-severity Prisma/deepmerge findings (`@prisma/config`, `prisma`, and `deepmerge-ts`), including [GHSA-ggr8-5vv4-36mx](https://github.com/advisories/GHSA-ggr8-5vv4-36mx) | Upgrade path must be tested against the Prisma 7 adapter/configuration; the automatic audit fix proposes a breaking path and no remediation is committed |
