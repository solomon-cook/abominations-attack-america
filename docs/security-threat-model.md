...
| HTTP/WebSocket abuse | Burst traffic, handshake storms, slow or oversized requests, resource exhaustion | Separate process-local HTTP and WebSocket limiters, 64 KiB JSON body cap, 15 s request/keep-alive timeout, 20 s header timeout, security headers, token-free metrics | Distributed limiter and edge rate policy; verify timeout behaviour at the deployed proxy |
