...
function UnitReference({ children, compactDeployment }: { children: ReactNode; compactDeployment: boolean }) {
  return compactDeployment ? <details className="unit-reference-details"><summary>Unit details</summary>{children}</details> : <>{children}</>;
}

export function MilitaryReference({ sheet, game, choices, onSelect, compactDeployment = false }: { sheet: string; game?: GameState; choices?: readonly DeploymentChoice[]; onSelect?: (choice: DeploymentChoice) => void; compactDeployment?: boolean }) {
  ...
  return <section className="sheet-reference" aria-label={`${sheet} reference rules`}>
    ...
    {units.map((unit) => <article key={unit.id}>
      ...
      <UnitReference compactDeployment={compactDeployment}><SheetStats values={{ Move: unit.id === "navy-nuclear-submarine" ? "4 / 8 as missile" : unit.move, Movement: movementLabel(unit.movement), Attacks: unit.attacks, Defense: printedValue(unit.defense), Damage: printedValue(unit.damage) }} />{unit.specialAbilityText && <p>{unit.specialAbilityText}</p>}</UnitReference>
    </article>)}
    {sheet === "National Guard" && <>
      <p className="deployment-reference"><b>Deploy:</b> An unstomped city, military base, or Infamy site.</p>
      {NATIONAL_GUARD_DEFINITIONS.map((unit) => <article key={unit.id}><h3>{unit.name} <small>· {unit.quantity} pieces</small></h3><ReserveSlots typeId={unit.id} quantity={unit.quantity} game={game} choices={choices} onSelect={onSelect} /><UnitReference compactDeployment={compactDeployment}><SheetStats values={{ "Printed move": unit.printedMove, "With Guard Commander": unit.move, Movement: unit.name === "Fighter" ? "Fly with commander" : "Land with commander", Attacks: 1, Defense: unit.defense, Damage: unit.damage }} /><p>{unit.specialAbilityText}</p></UnitReference></article>)}
