...
- [x] [P0] Show connection, reconnecting, offline, stale, and unrecoverable session states. (Web lobby renders connection state; session restore and command failures retain explicit error text)
  - [x] Define session expiry, revocation, token rotation, and room access policy. (`docs/room-lifecycle.md` defines bearer-token scope, 24-hour expiry/revocation-by-expiry, disconnect behavior, role/seat/revision boundaries, and the implemented `rotate-session` store/API primitive; audit delivery and automatic/external identity rotation remain release work.)
