...
- [ ] [P0] Add a board-comparison evidence pack with annotated full-board imagery, coordinate review, and human sign-off before enabling MVP play. (Generated `docs/board-comparison-overlay.svg` and `docs/board-top-down-overlay.svg` now provide perspective-aware cross-checks over both the 2840 × 1752 setup photograph and the independent 753 × 623 top-down photograph, with checked-in `npm run board-overlay:verify` regeneration checks; `docs/board-promotion-signoff.md` now records the required cell, edge, source, and reviewer approvals. The sources remain partly occluded/low-resolution, so human transcription sign-off and a sufficiently legible aligned source remain outstanding.)
  - [x] Maintain a provisional 254-cell transcription ledger with explicit low-confidence states and a validator that prevents it from becoming production board data. (`docs/provisional-board-transcription.md`, `npm run board-transcription:verify`; cell-level source review and human sign-off remain open.)
...
- [ ] [P1] Add browser compatibility and responsive visual-regression coverage. (Chrome runtime matrix and deterministic baselines pass; the macOS `browser-safari` CI job now creates a Safari session and passes the 254-cell/two-reference board-review smoke, while Firefox/Edge coverage and manual cross-engine acceptance remain open.)
