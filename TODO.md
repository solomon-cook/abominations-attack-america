...
- [ ] [P0] Add a board-comparison evidence pack with annotated full-board imagery, coordinate review, and human sign-off before enabling MVP play. (Generated `docs/board-comparison-overlay.svg` and `docs/board-top-down-overlay.svg` now provide perspective-aware cross-checks over both the 2840 × 1752 setup photograph and the independent 753 × 623 top-down photograph, with checked-in `npm run board-overlay:verify` regeneration checks; the sources remain partly occluded/low-resolution, so human transcription sign-off and a sufficiently legible aligned source remain outstanding.)
  ...
  - [ ] [P0] Replace the current screenshot-like reconstruction with a source-faithful player board before calling it implemented. The supplied comparison image exposes these concrete failures:
    ...
    - [ ] [P0] Remove `Unresolved <coordinate>` labels and placeholder dots from every player-facing playable hex; each visible cell must show its authored identity or an intentionally blank/sea/boundary treatment backed by reviewed board data. (Unreviewed shell cells now omit visible labels, dots, and terrain artwork; their coordinate/review state remains in accessible metadata. Authored physical identities remain source-gated.)
    - [ ] [P0] Replace the rectangular candidate-shell presentation with the physical board's verified land/sea/lake/coast/boundary silhouette, disabled cells, and off-board edges; the decorative CSS background must never be mistaken for board topology. (Unresolved shell cells now use a neutral review-pending treatment rather than implied coast/land artwork; the verified physical silhouette and disabled-edge data remain outstanding.)
    ...
    - [ ] [P0] Make visual geometry match the authored lattice: no overlapping or clipped hexes, no feature labels or pieces spilling into neighbouring cells, consistent visible gaps, and stable alignment at fit, zoom, desktop, tablet, and mobile sizes. (The candidate 254-cell lattice and sparse development fixture now use separated flat-top hex geometry with source contracts for overlap/layout; physical-board silhouette and manual viewport acceptance remain source/QA-gated.)
      ...
      - [x] [P0] Render the seven off-shell development compatibility locations as compact, explicitly styled fixture pins rather than a second full-size hex lattice, so they cannot overlap or be mistaken for physical board cells.
