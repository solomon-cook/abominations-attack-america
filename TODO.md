...
- [ ] [P0] Add end-to-end browser tests for local first playable and two-browser online play.
  ...
  - [x] Add a reproducible three-session Chrome CDP smoke harness for room create/join, spectator join, synchronized setup, Ready, Move phase, spectator no-act projection, one forged-command rejection, one malformed-command rejection, both without state change, reload recovery, WebSocket/browser-process disconnect and reconnect recovery, synchronized legal movement, Encounter decision chain, and Deploy pass (`scripts/verify-browser-online.mjs`, `npm run browser:online:verify`; victory and production-board evidence remain open).
