...
- [x] [P1] Add Monster Challenge opponent selection, weigh-in, duel progression, and final victory presentation. (Opponent selection, weigh-in, monster/giant duel progression, both terminal outcomes, and a stylized split-screen result surface are implemented in the challenge-enabled development ruleset; exact source-art direction remains.)
  - [ ] [P1] Replace the final Monster Challenge board view with an anime-style split-screen duel: one monster on each half, the dice roll presented in the centre, and turn-by-turn Health drops clearly animated after each authoritative result. (The source-backed panel provides the split-screen, centre dice, sequential Health replay, and an original CSS impact treatment; exact anime artwork remains a content/IP decision.)
