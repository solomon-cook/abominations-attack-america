...
- [x] [P0] Implement a separately versioned `PROVISIONAL_AUTHORITATIVE_BOARD` for local/test play from the promoted row/column dataset in [`docs/provisional-board-transcription.md`](docs/provisional-board-transcription.md#promoted-playtest-transcription-for-implementation-handoff), using the current 254-cell axial shell and explicitly labelling the ruleset as a promoted playtest guess rather than physically verified. (`PROVISIONAL_AUTHORITATIVE_BOARD` pins 254 provisional cells, guessed feature assignments, enabled shell adjacency, a distinct content hash, and a `playtest-0.2-promoted-guess` ruleset; production validation rejects every provisional cell.)
