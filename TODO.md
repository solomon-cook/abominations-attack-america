...
- [x] [P0] Implement a separately versioned `PROVISIONAL_AUTHORITATIVE_BOARD` for local/test play from the promoted row/column handoff recorded in the preserved local review artifact and summarized in [`docs/full-honeycomb-board-evidence.md`](docs/full-honeycomb-board-evidence.md), using the current 254-cell axial shell and explicitly labelling the ruleset as a promoted playtest guess rather than physically verified. (`PROVISIONAL_AUTHORITATIVE_BOARD` pins 254 provisional cells, guessed feature assignments, enabled shell adjacency, a distinct content hash, and a `playtest-0.2-promoted-guess` ruleset; `Play provisional honeycomb` exposes the same pinned board through the local browser; production validation rejects every provisional cell.)
  ...
  - [x] [P0] Add a named test factory/selector so local board playtests pin the provisional board ID, version, ruleset version, and content hash, while production room creation continues to reject it. (`createProvisionalPlaytestGame` and `boardForState` pin and resolve the provisional identity; `assertMvpBoardReady` continues to validate only the physically reviewed full-board definition.)
...
- [x] [P0] Make production validation fail for unresolved rule-bearing board fields. (Production validation rejects unresolved hex verification, water classes, and edge barriers; board tests prove the development and full-board fixtures are not production-ready.)
...
- [ ] [P0] Two new players can complete the simplified first-playable flow in supported desktop and mobile browsers without developer assistance. (The browser labels the sparse path as `Development playtest` and exposes a separate `Play provisional honeycomb` local path; online production room creation correctly fails closed until the physically verified board is promoted.)
