...
- [x] [P0] Implement a separately versioned `PROVISIONAL_AUTHORITATIVE_BOARD` for local/test play from the promoted row/column handoff recorded in the preserved local review artifact and summarized in [`docs/full-honeycomb-board-evidence.md`](docs/full-honeycomb-board-evidence.md), using the current 254-cell axial shell and explicitly labelling the ruleset as a promoted playtest guess rather than physically verified. (`PROVISIONAL_AUTHORITATIVE_BOARD` pins 254 provisional cells, guessed feature assignments, enabled shell adjacency, a distinct content hash, and a `playtest-0.2-promoted-guess` ruleset; production validation rejects every provisional cell.)
