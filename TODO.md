...
- [ ] [P0] Add a board-comparison evidence pack with annotated full-board imagery, coordinate review, and human sign-off before enabling MVP play. (Generated `docs/board-comparison-overlay.svg` and `docs/full-honeycomb-board-evidence.md` now provide the annotated 254-cell/photo comparison, including a perspective-aware overlay over the 2840 × 1752 full-board setup photograph and a checked-in `npm run board-overlay:verify` regeneration check; the source is still partly occluded, so human transcription sign-off and a sufficiently legible aligned source remain outstanding.)
