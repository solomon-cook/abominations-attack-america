...
- [ ] [P1] Draw and immediately apply a Mutation card on first use of a site. (First-use site draws now produce a structured `mutationDraws` status in the authoritative encounter event, are shown in the encounter result surface, and are recorded face up in the monster player's hand; independently implemented effects are active immediately, while unresolved card effects remain source-gated.)
