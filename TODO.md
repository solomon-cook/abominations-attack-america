...
- [ ] [P0] Browser verification covers every first-playable decision, cancellation, invalid action, loading state, and victory state.
  - [x] Extend the reproducible local development smoke through the deterministic fixture's authored Denver Fight, source-backed Encounter decision, and Deploy pass; verify unreachable destinations remain disabled, the pending-action loading state is visible, and the authoritative concession terminal summary renders; malformed-command, temporary-victory, and full-board acceptance remain open.
  - [ ] [P0] Make the temporary-victory browser scenario legally reachable from a normal development playtest before claiming victory evidence. The current two-player fixture starts with 14 Stomp markers but exposes only seven stompable spaces, while development-board exhaustion also requires the two non-stompable Mutation/Challenge spaces; either correct the fixture/ruleset boundary or provide a documented authoritative browser fixture with enough legal encounters, then prove the terminal winner through rendered controls only.
...
- [ ] [P0] Add end-to-end browser tests for local first playable and two-browser online play.
  ...
  - [x] Add a reproducible three-session Chrome CDP smoke harness for room create/join, spectator join, synchronized setup, Ready, Move phase, spectator no-act projection, one forged-command rejection without state change, reload recovery, synchronized legal movement, Encounter decision chain, and Deploy pass (`scripts/verify-browser-online.mjs`, `npm run browser:online:verify`; disconnect/reconnect, victory, and production-board evidence remain open).
