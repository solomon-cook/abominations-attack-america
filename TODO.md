...
- [ ] [P0] Browser verification covers every first-playable decision, cancellation, invalid action, loading state, and victory state.
  - [x] Extend the reproducible local development smoke through the deterministic fixture's authored Denver Fight, source-backed Encounter decision, and Deploy pass; verify unreachable destinations remain disabled and the pending-action loading state is visible; malformed-command, victory, and full-board acceptance remain open.
