...
- [ ] [P0] Add a board-comparison evidence pack with annotated full-board imagery, coordinate review, and human sign-off before enabling MVP play. (Generated `docs/board-comparison-overlay.svg` and `docs/board-top-down-overlay.svg` now provide perspective-aware cross-checks over both the 2840 × 1752 setup photograph and the independent 753 × 623 top-down photograph, with checked-in `npm run board-overlay:verify` regeneration checks; the sources remain partly occluded/low-resolution, so human transcription sign-off and a sufficiently legible aligned source remain outstanding.)
  ...
  - [ ] [P0] Replace the current screenshot-like reconstruction with a source-faithful player board before calling it implemented. The supplied comparison image exposes these concrete failures:
    ...
    - [ ] [P0] Remove every screenshot/reconstruction artefact from the player surface: repeated placeholder dots, `Unresolved <coordinate>` text, guessed terrain colouring, arbitrary rectangular clipping, and feature pins positioned by visual approximation rather than canonical hex keys.
