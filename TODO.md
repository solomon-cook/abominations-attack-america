...
| Browser client | Partial prototype | React/Vite renders local and online turns, room controls, phase prompts, and a log on the pinned best-guess 254-cell MVP honeycomb; verified physical-board fidelity and full-rules release scope remain incomplete. |
...
| Board | Best-guess MVP; verified-release blocker | The pinned `PROVISIONAL_AUTHORITATIVE_BOARD` supplies a staggered 13-row, 254-cell honeycomb with provisional markers and enabled guessed edges for MVP play; physical transcription, exact water/barrier data, and human sign-off remain incomplete. |
...
- [x] [P0] Author every playable board hex with a stable key, coordinate, best-guess water class, label where legible, features, source references, and verification status. (`PROVISIONAL_AUTHORITATIVE_BOARD` now supplies all 254 MVP cells; physical corrections remain versioned follow-up work.)
...
- [x] [P0] Replace the sparse nine-space development topology with the fully filled best-guess honeycomb board as the MVP playable board. The development fixture remains for isolated tests; MVP room creation now pins the 254-cell provisional board, while physically verified release promotion remains a separate gate.
  ...
  - [x] [P0] Make the fully filled honeycomb board an explicit first-playable MVP gate; sparse nine-space topology is development-only and cannot be used by an MVP room. (`assertMvpBoardReady`/`createMvpRoomGame` validate and pin `PROVISIONAL_AUTHORITATIVE_BOARD`; physically verified release validation remains stricter.)
...
- [ ] [P0] Complete the board-comparison evidence pack for verified physical-board promotion, with annotated full-board imagery, coordinate review, and human sign-off. (`docs/board-comparison-overlay.svg` and `docs/board-top-down-overlay.svg` provide perspective-aware cross-checks over both the 2840 × 1752 setup photograph and the independent 753 × 623 top-down photograph, with checked-in `npm run board-overlay:verify` regeneration checks; `docs/board-promotion-signoff.md` records the required cell, edge, source, and reviewer approvals. The best-guess MVP is enabled separately; physical transcription sign-off and a sufficiently legible aligned source remain outstanding.)
  - [x] Maintain a provisional 254-cell transcription ledger with explicit low-confidence states and a validator that prevents it from becoming production board data. (`docs/provisional-board-transcription.md`, `npm run board-transcription:verify`; the verifier warns on clean checkouts where the preserved local review artifact is absent and supports strict enforcement with `REQUIRE_PROVISIONAL_BOARD_LEDGER=1`; cell-level source review and human sign-off remain open.)
  ...
  - [ ] [P0] Replace the current screenshot-like reconstruction with a source-faithful player board before calling it implemented. The supplied comparison image exposes these concrete failures:
    ...
    - [ ] [P0] Pass a same-scale visual comparison against the supplied physical-board reference before promotion: the rendered result must read as one fully filled honeycomb board, not a rectangular grid of generic spaces. Review the 13-row alternating cell pattern, outer silhouette, row offsets, coast/sea cut-outs, and off-board boundary against the reference image.
    - [ ] [P0] Use the separated cream hex-face treatment for each playable land face, with a uniform visible seam/gap between neighbouring faces. A face must never overlap its neighbour, expose a second rotated lattice, or be stacked using per-tile rotation plus compensating board transforms.
    - [ ] [P0] Keep all tile art, labels, markers, cities, bases, and units inside the authored face that owns them. No feature decal, piece, label, or interaction target may float across a seam or visually belong to a neighbouring hex.
    ...
    - [ ] [P0] Verify that the board remains landscape and correctly stacked after fit, pan, zoom, desktop, tablet, and mobile rendering. The implementation must use one flat-top coordinate system and row-level staggering; rotating individual tiles or rotating the whole board to repair stacking is not an acceptable production fix.
    - [ ] [P0] Remove `Unresolved <coordinate>` labels and placeholder dots from every player-facing playable hex; each visible cell must show its authored identity or an intentionally blank/sea/boundary treatment backed by reviewed board data. (Unreviewed shell cells now use separated neutral cream faces without visible labels, feature art, or gameplay controls; their coordinate/review state remains in accessible metadata. Authored physical identities remain source-gated.)
    - [ ] [P0] Replace the rectangular candidate-shell presentation with the physical board's verified land/sea/lake/coast/boundary silhouette, disabled cells, and off-board edges; the decorative CSS background must never be mistaken for board topology. (The development match now visibly renders all 254 separated cream candidate faces with only nine canonical development overlays enabled; unresolved shell cells remain neutral and disabled, while the verified physical silhouette and disabled-edge data remain outstanding.)
    ...
    - [ ] [P0] Make visual geometry match the authored lattice: no overlapping or clipped hexes, no feature labels or pieces spilling into neighbouring cells, consistent visible gaps, and stable alignment at fit, zoom, desktop, tablet, and mobile sizes. (The candidate 254-cell lattice and sparse development fixture now use separated flat-top hex geometry; source and Chrome runtime contracts verify canonical width, row staggering, same-row non-overlap, and all three smoke viewports. Physical-board silhouette and manual viewport acceptance remain source/QA-gated.)
      ...
      - [x] [P0] Add a visual geometry regression fixture that asserts adjacent shared edges, row pitch, tile containment, and zero unintended intersections at the supported aspect ratios before source-board promotion. (`scripts/verify-web-board-layout.ts` now checks polygon-level interior intersections across all 254 candidate cells.)
...
### Provisional-authority playtest bridge — use the current guesses now, verify the physical board later
  ...
  - [x] [P0] Implement a separately versioned `PROVISIONAL_AUTHORITATIVE_BOARD` for MVP play from the promoted row/column handoff recorded in the preserved local review artifact and summarized in [`docs/full-honeycomb-board-evidence.md`](docs/full-honeycomb-board-evidence.md), using the current 254-cell axial shell and explicitly labelling the ruleset as a promoted playtest guess rather than physically verified. (`PROVISIONAL_AUTHORITATIVE_BOARD` pins 254 provisional cells, guessed feature assignments, enabled shell adjacency, a distinct content hash, and a `playtest-0.2-promoted-guess` ruleset; MVP room creation and `Play provisional honeycomb` use the same pinned board; the verified-release validator still rejects provisional cells.)
  ...
  - [x] [P0] Add a named test factory/selector so local and MVP board playtests pin the provisional board ID, version, ruleset version, and content hash, while verified-release validation continues to reject it. (`createProvisionalPlaytestGame`, `createMvpRoomGame`, and `boardForState` pin and resolve the provisional identity; `validateBoardDefinition(..., { production: true })` remains the strict physical-release boundary.)
  ...
  - [x] [P0] Add structural tests for 254 unique hexes, reciprocal edges, feature counts, provisional verification status, pinned identity, movement, city encounters, base encounters, and skipped stomped cells. (`board.test.ts` and `index.test.ts` cover each boundary.)
...
- [x] [P0] Validate complete regular-unit inventory accounting before the first turn begins. (Source-counted regular quantities, structural IDs, positions, battle references, National Guard collisions, and movement ledgers are validated at creation, before commands, and after every successful event transition.)
  ...
  - [ ] [P1] Add lobby controls for player count, ready state, seat order, display names, and room privacy. (Player count, fixed seat order, readiness, display names, and private/public room choice are exposed through the lobby and authoritative APIs; private rooms reject spectator entry and default private. Public discovery and invite-link policy remain open.)
...
- [x] [P0] Split the monolithic prototype into maintainable lobby, setup, board, turn, battle, card, log, and end-game surfaces. (The web surface is decomposed into dedicated lobby, setup, board, turn, battle, card, log, and terminal components; command authority remains in `App`/the engine.)
  ...
  - [x] [P0] Create a clean, art-led Home Screen that gives players an immediate, uncluttered entry point into the game. (`apps/web/src/components/HomeScreen.tsx`; full verification passes.)
    ...
    - [x] [P0] Render the board as a connected proper game-board layout of hexagons, with a consistent small border/gap between neighbouring hexes so the board structure is immediately legible. (Flat-top landscape tiles use a mathematically shared-edge 75% centre pitch, staggered rows, contained labels, and the `web-board-layout:verify` contract.)
  ...
  - [x] [P0] Render all board hexes, features, barriers, labels, and pieces from the authoritative board and match state. (The renderer consumes the pinned canonical board for MVP rooms and renders all 254 best-guess honeycomb cells; physically verified content and final feature corrections remain source-gated.)
...
- [ ] [P0] Complete the Civ VI-style information-dense board experience so the player remains in the full-screen game view while detail appears contextually rather than through a separate “show controls” mode.
  ...
  - [ ] Convert secondary information into compact board-adjacent trays, anchored popovers, expandable cards, or modal/lightbox surfaces that preserve visible board context.
  ...
  - [ ] Make adjacency visibly meaningful through connected borders, movement range, attack range, flanking/position states, encounter relationships, and neighbouring bonuses where supported by the rules.
  ...
  - [ ] Use deliberate full-screen mode changes only for exceptional moments such as the Monster Challenge, with an explicit return to the board afterward.
...
- [x] [P0] Add first-playable setup, gameplay, victory, restart, and error states. (Local browser flow now exposes source-gated development setup, persists completed assignments into `GameState`, and retains gameplay/victory/restart/error states; production setup remains blocked by unresolved source data)
  ...
  - [x] [P1] Add private card zones, public persistent effects, timing prompts, and target selection. (Player projections expose only the viewer's face-up card zone; spectators and other players receive redacted hands, while the browser labels revealed cards and source-gated effects explicitly.)
    - [ ] [P1] Let players view the cards in their own hand and play any card whose effect is not passive or immediate, with legal timing, targets, confirmation, and authoritative resolution shown in the UI. (Private-hand cards expose availability, classification, timing, duration, source, target/confirmation boundaries, result handling, and persistent-effect status; Berserk, Son of a Monster, and Defense Satellites expose direct hand actions when their authoritative windows are legal; Blonde Lure, Antimatter, Stabilizer Ray, Laser Fence, X-Fighters, and giant placement expose their phase-specific timing surfaces, while source-gated effects remain unavailable.)
...
- [ ] [P0] Two new players can complete the simplified first-playable flow in supported desktop and mobile browsers without developer assistance. (Normal online MVP room creation now uses the pinned best-guess 254-cell honeycomb; the browser labels its provisional status, while mobile and full human playthrough evidence remain open.)
- [ ] [P0] The first-playable browser flow renders and uses the fully filled best-guess authoritative honeycomb board; the sparse nine-space topology is not exposed as an MVP match option. (The normal online path now pins `PROVISIONAL_AUTHORITATIVE_BOARD`; browser completion evidence remains open.)
...
- [x] [P0] Add end-to-end browser tests for local first playable and two-browser online play. (The local and three-session Chrome harnesses cover setup, movement, Fight, Encounter, Deploy, invalid/forged commands, loading, reconnect, terminal projection, and development-fixture victory/concession; full-board and production acceptance remain separate release gates.)
  ...
  - [x] Add a reproducible three-session Chrome CDP smoke harness for room create/join, spectator join, synchronized setup, Ready, Move phase, spectator no-act projection, one forged-command rejection, one malformed-command rejection, both without state change, reload recovery, WebSocket/browser-process disconnect and reconnect recovery, synchronized legal movement, a legal Fight decision chain, Encounter decision chain, Deploy pass, current-player concession, synchronized terminal Victory projection, and terminal reload recovery (`scripts/verify-browser-online.mjs`, `npm run browser:online:verify`; development-fixture evidence only; full-rules victory, persistence across process restart, and production-board evidence remain open).
  ...
  - [x] Run a two-tab online setup/reload smoke and record the result: the earlier partial run left movement unverified, but the later three-session harness now verifies synchronized legal movement, Encounter, Deploy, concession, terminal projection, and reload recovery; Fight progression, full-rules victory, persistence across process restart, and production-board evidence remain open.
...
- [ ] [P1] Add browser compatibility and responsive visual-regression coverage. (Chrome runtime matrix and deterministic baselines pass; GitHub Actions run `32942215545` passed Chrome, macOS Safari, Ubuntu Firefox, and Ubuntu Edge browser jobs, while manual cross-engine acceptance remains open.)
