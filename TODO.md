...
- [ ] [P1] Build a composable effect system for stat modifiers, movement abilities, attack changes, control overrides, placement, and triggered effects. (A typed `EFFECT_BOUNDARIES` registry now covers all six categories and the shared accumulator composes implemented continuous Mutation/Research effects; optional, triggered, placement, and source-gated lifecycles remain explicitly bounded, with unresolved production behaviour still unavailable.)
