...
await page.locator('.record-reserve-slots button').first().click();
...
await page.mouse.move(650,450);await page.mouse.down();await page.mouse.move(930,310,{steps:12});await page.mouse.up();
const ownedArt=page.locator('img.tile-occupant[alt^="Navy"]');
await ownedArt.first().click();
const detail=page.getByRole('region',{name:'Selected piece details'});await detail.scrollIntoViewIfNeeded();
assert.ok(await detail.locator('img').evaluate(i=>i.complete&&i.naturalWidth>0));
assert.match(await detail.innerText(),/Movement:/);
await page.screenshot({path:'tmp/unit-detail-selected.png'});
